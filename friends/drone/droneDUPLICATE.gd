...
# Zmienne do kontroli kierunku patrzenia drona
var direction_timer = 0.0
var min_direction_time = 1.0  # Minimalny czas patrzenia w jednym kierunku (w sekundach)
var current_direction = 1      # 1 = prawo, -1 = lewo
...
func _ready():
	...
	print("Czy ShootingStart istnieje: ", has_node("%ShootingStart"))
...
func _physics_process(delta):
	...
	# Aktualizacja licznika czasu kierunku
	direction_timer += delta

	# Logika obracania sprite'a z minimalnym czasem utrzymania kierunku
	if sprite_node:
		# Sprawdź, czy dron porusza się wystarczająco szybko, aby zmienić kierunek
		if velocity.length() > 5.0:
			var desired_direction = 1 if velocity.x > 0 else -1 if velocity.x < 0 else current_direction
			
			# Zmień kierunek tylko jeśli minął minimalny czas
			if desired_direction != current_direction and direction_timer >= min_direction_time:
				current_direction = desired_direction
				direction_timer = 0.0  # Resetuj timer po zmianie kierunku
				
				# Zastosuj nowy kierunek do sprite'a
				sprite_node.flip_h = (current_direction < 0)
	else:
...
func rotate_shoot_point_to_target():
	if target and is_instance_valid(target):
		var shoot_point = get_node_or_null("%ShootingStart")
		if shoot_point:
			...
			print("OSTRZEŻENIE: Nie znaleziono węzła %ShootingStart w dronie.")
...
func shoot():
	...
	var shoot_point = get_node_or_null("%ShootingStart")
	if not shoot_point:
		print("Brak punktu wystrzału (%ShootingStart)!")
		return
