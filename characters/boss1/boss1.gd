...
func _ready():
	# Dodaj bossa do grupy enemy, aby drony mogły go wykrywać
	add_to_group("enemy")
...
func _on_hit_area_body_entered(body):
	if body == player:
		...
		damage_timer.start()
		print("Boss: Kolizja z graczem - start zadawania obrażeń")
	elif body.is_in_group("alliance"):
		# Rozpocznij zadawanie obrażeń przyjacielowi
		current_target = body
		damage_timer.start()
		print("Boss: Kolizja z przyjacielem - start zadawania obrażeń")

func _on_hit_area_body_exited(body):
	if body == current_target:
		damage_timer.stop()
		current_target = null
		print("Boss: Wyjście z obszaru - stop zadawania obrażeń")
