...
# Zmienna do przechowywania aktualnego celu ataku (gracz lub przyjaciel)
...
var charge_duration = 2.0  # ile czasu trwa faktyczna szarża
...
func _ready():
	...
	# Dodaj się do grupy enemy, aby drony mogły cię wykrywać
	add_to_group("enemy")
...
func _on_hit_area_body_entered(body):
	if body == player:
		...
		# Rozpocznij zadawanie obrażeń przyjacielowi
		current_target = body
		damage_timer.start()

func _on_hit_area_body_exited(body):
	if body == player or body == current_target:
		damage_timer.stop()
...
func _on_damage_timer_timeout():
	...
	elif player and current_target == null:
...
func start_charge():
	...
	velocity = Vector2.ZERO
	move_and_slide()  # by wróg faktycznie zatrzymał się w tej klatce
