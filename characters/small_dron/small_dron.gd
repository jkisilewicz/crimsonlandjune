...
func _ready():
	# Dodaj drona do grupy enemy, aby przyjacielskie drony mogły go wykrywać
	add_to_group("enemy")
...
func _physics_process(_delta):
	...
	if direction.x < 0:
		%RoundEnemyBody.flip_h = true  
		%RoundEnemyBody.flip_h = true  
	else:
		%RoundEnemyBody.flip_h = false 
...
func _on_hit_area_body_entered(body):
	if body == player:
		...
		damage_timer.start()
		print("Mały Dron: Kolizja z graczem - start zadawania obrażeń")
	elif body.is_in_group("alliance"):
		current_target = body
		damage_timer.start()
		print("Mały Dron: Kolizja z przyjacielem - start zadawania obrażeń")

func _on_hit_area_body_exited(body):
	if body == current_target:
		damage_timer.stop()
		current_target = null
		print("Mały Dron: Wyjście z obszaru - stop zadawania obrażeń")
